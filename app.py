...
from riscos import funcoes_riscos
...
def buscar_requisitos(cargo_id):
    exames_response = db.execute_query(
        "SELECT nome FROM exames WHERE id IN (SELECT exame_id FROM cargo_exame WHERE cargo_id = ?)",
        (cargo_id,),
    )
    ...
    epis = [
        {"nome": row[0], "unidade": row[1], "quantidade": row[2]}
        for row in epis_response
    ]

    integracoes_response = db.execute_query(
        "SELECT nome FROM integracoes WHERE id IN (SELECT integracao_id FROM cargo_integracao WHERE cargo_id = ?)",
        (cargo_id,),
    )
...
db = Database()
pdf_epi = EPI_PDF()
pdf_aso = ASO_PDF(dados_cnpj, funcoes_riscos)
