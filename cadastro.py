...
def atualizar_requisitos(cargo_id, exames, epis, integracoes):
    ...
    for epi_id, quantidade in epis:
        db.execute_query(
            "INSERT INTO cargo_epi (cargo_id, epi_id, quantidade) VALUES (?, ?, ?)",
            (cargo_id, epi_id, quantidade),
        )

    for integracao_id in integracoes:
        db.execute_query(
            "INSERT INTO cargo_integracao (cargo_id, integracao_id) VALUES (?, ?)",
            (cargo_id, integracao_id),
        )
...
exames_selecionados, epis_selecionados, integracoes_selecionadas = buscar_requisitos(
    cargo_id
)
...
with col1:
    st.subheader("Exames")
    exames_checkbox = [
        st.checkbox(exame, value=(i + 1 in exames_selecionados))
        for i, exame in enumerate(exames_disponiveis)
    ]
...
with col2:
    ...
    epi_quantidades_salvas = {
        epi_id: quantidade for epi_id, quantidade in epis_selecionados
    }
...
with col3:
    ...
    integracoes_checkbox = [
        st.checkbox(integracao, value=(i + 1 in integracoes_selecionadas))
        for i, integracao in enumerate(integracoes_disponiveis)
    ]

if st.button("Atualizar"):
    exames_ids = [i + 1 for i, checked in enumerate(exames_checkbox) if checked]
    epis_ids_quantidades = [
        (i + 1, quantidade)
        for i, (checked, quantidade) in enumerate(zip(epis_checkbox, epis_quantidades))
        if checked
    ]
    integracoes_ids = [
        i + 1 for i, checked in enumerate(integracoes_checkbox) if checked
    ]

    atualizar_requisitos(
        cargo_selecionado, exames_ids, epis_ids_quantidades, integracoes_ids
    )
